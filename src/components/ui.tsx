    ...
    >
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          {trend && (
            <div className={`text-[11px] mt-1.5 flex items-center gap-1 font-semibold ${trend.up ? 'text-emerald-600' : 'text-red-600'}`}>
  ...
  return (
    <div className="gov-card p-0 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-200 bg-white flex items-center justify-between">
        <div>
          <h3 className="m-0 text-[13px] font-bold text-slate-900">{title}</h3>
          {subtitle && <p className="m-0 text-[11px] text-slate-500 mt-0.5">{subtitle}</p>}
        </div>
...
export function FilterBar({ filters }: FilterBarProps) {
  return (
    <div className="gov-card py-2.5 px-4 flex flex-wrap gap-3 items-center mb-4">
      <span className="text-[11.5px] font-bold text-slate-600 uppercase tracking-wider">Filter By:</span>
      {filters.map(f => (
        <div key={f.label} className="flex items-center gap-1.5">
          ...
          <select
            ...
            className="border border-slate-300 rounded px-2 py-1 text-xs bg-white text-slate-700 cursor-pointer focus:outline-none focus:border-blue-500"
          >
  ...
  return (
    <div className="mb-4.5">
      <div className="flex items-center gap-2.5 mb-0.5">
        <h2 className="m-0 text-[19px] font-bold text-[#003580] tracking-tight">{title}</h2>
        {badge}
      </div>
      {subtitle && <p className="m-0 text-[12.5px] text-slate-500">{subtitle}</p>}
    </div>
  );
}
...
export function StatusBadge({ status }: { status: string }) {
  const statusClasses: Record<string, string> = {
    ...
    'Sanctioned': 'bg-indigo-50 text-indigo-800 border-indigo-200',
    'Recommended': 'bg-slate-50 text-slate-700 border-slate-200',
    'Non-Progress': 'bg-red-50 text-red-800 border-red-200',
    'Dropped': 'bg-red-50 text-red-900 border-red-200',
      ...
      {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
        <button
          ...
          className={`min-w-[26px] h-[26px] rounded px-1.5 text-[11.5px] cursor-pointer font-medium border transition-colors ${
            p === page
              ? 'bg-[#003580] text-white border-[#003580] font-bold'
...
export function ActionButton({ label, onClick, variant = 'primary' }: {
  ...
  const variantClasses: Record<string, string> = {
    primary: 'bg-[#003580] text-white border-[#003580] hover:opacity-90',
    secondary: 'bg-white text-[#003580] border-[#003580] hover:bg-slate-50',
    danger: 'bg-red-600 text-white border-red-600 hover:bg-red-700',
    ghost: 'bg-transparent text-slate-700 border-slate-300 hover:bg-slate-100',
  };
  return (
    <button
      onClick={onClick}
